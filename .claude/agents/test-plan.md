...
| **Architecture** | **Repository Pattern** | **Application Layer는 Domain의 Interface만 의존**하며, Infrastructure 구현체(Isar)를 절대 직접 참조하지 않음. | Supabase/클라우드 전환 리스크 격리. |
...
| **Infrastructure (10%)**| **Integration** | IsarRepository, DTO, Service | **`isar_test`** 또는 `Isar.open` (in-memory) 활용. | **DB Unique 제약 조건 및 트랜잭션**의 무결성 검증. DTO ↔ Entity 간 **데이터 직렬화/역직렬화(Serialization)**의 정확성. |
...
| **F001** (스케줄) | IT-002 (Rollback) | `isar_test` + `fake_async` | 투여 계획 수정(UF-009) 실패 시 **DB 롤백** 및 **스케줄 재계산 1초 이내 완료** 확인. |
