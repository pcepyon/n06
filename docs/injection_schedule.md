...
- 연체 제한 **미적용** (항상 기록 가능)
- "과거 기록" 배지 표시 + "이 날짜에 투여했다면 기록하세요" 안내
- 이미 투여했지만 기록하지 않은 경우를 위한 기능

### 스케줄 연결 규칙

스케줄 없는 날짜에 기록 시 가장 가까운 미완료 스케줄에 연결:

- 거리가 다르면 → **가까운 스케줄** 우선
- 거리가 같으면 → **과거 스케줄** 우선 (밀린 것 먼저 처리)
