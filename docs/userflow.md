...
8. 내부 DB에 사용자 계정 정보 저장 또는 업데이트 (auth_type: 'social')
...
#### 처리
1. 로그인 화면에서 "이메일로 시작하기" 버튼 표시
2. 버튼 클릭 시 회원가입/로그인 선택 화면으로 이동
3. "회원가입" 선택 시 회원가입 폼 화면 표시
4. 이메일 입력 필드:
   - 실시간 이메일 형식 검증 (정규식: RFC 5322)
   - 유효하지 않은 형식 시 에러 메시지 표시
   - 포커스 아웃 시 중복 이메일 확인 (API 호출)
5. 비밀번호 입력 필드:
   - 실시간 비밀번호 강도 검증
   - 최소 8자, 영문+숫자 조합 체크
   - 강도 표시: 약함(빨강)/보통(주황)/강함(초록)
   - 조건 미충족 시 에러 메시지 표시
6. 비밀번호 확인 입력 필드:
   - 실시간 일치 여부 검증
   - 불일치 시 에러 메시지 표시
7. 동의 체크박스:
   ...
   - 개인정보처리방침 동의 (필수)
   - 미동의 시 회원가입 버튼 비활성화
8. 모든 검증 통과 시 회원가입 버튼 활성화
9. 회원가입 버튼 클릭:
   - 비밀번호 해싱 처리 (bcrypt)
   - 사용자 계정 생성 (auth_type: 'email')
   - 동의 정보 저장
   - Access/Refresh Token 발급 및 저장
   - 로그인 시도 횟수 0으로 초기화
10. 자동 로그인 처리 후 온보딩 화면으로 이동

#### 출력
- 실시간 입력 검증 피드백 (이메일 형식, 비밀번호 강도, 일치 여부)
- 이메일 중복 시 "이미 가입된 이메일입니다" 메시지 및 로그인 화면 이동 링크
- 회원가입 성공 시 온보딩 화면으로 전환
- 실패 시 에러 메시지 (네트워크 오류, 서버 오류 등)

#### 엣지케이스
- 이메일 중복: "이미 가입된 이메일입니다. 로그인하시겠습니까?" 안내
- 비밀번호 불일치: 실시간 빨간색 에러 표시
- 약한 비밀번호: 경고 표시하되 가입 허용 (최소 조건만 충족 시)
- 네트워크 오류: "네트워크 연결을 확인해주세요" 안내 및 재시도 버튼
- 입력 중 앱 종료: 임시 저장 없이 폐기
- 특수문자 포함 비밀번호: 허용 (선택사항)
...
6. "자동 로그인" 체크박스 제공
7. 로그인 버튼 클릭:
   - 이메일/비밀번호 비어있지 않은지 확인
   - 인증 요청 처리 (이메일 + 비밀번호 해시 비교)
8. 계정 잠금 확인:
   - locked_until 확인
   - 잠금 중이면 에러 메시지 표시 및 해제 시간 안내
9. 인증 성공 시:
   - Access/Refresh Token 발급
   - 토큰을 안전한 저장소에 저장
   - 로그인 시도 횟수 0으로 초기화
   - last_login_at 업데이트
   - 자동 로그인 체크 시 Refresh Token 유지
   - 홈 대시보드로 이동
10. 인증 실패 시:
   - 로그인 시도 횟수 +1
   - 5회 이상 시 계정 15분간 잠금 (locked_until 설정)
   - "이메일 또는 비밀번호가 올바르지 않습니다" 에러 메시지
   - 남은 시도 횟수 안내 (5회 중 N회 남음)

#### 출력
- 로그인 성공 시 홈 대시보드로 전환
- 실패 시 에러 메시지 ("이메일 또는 비밀번호가 올바르지 않습니다")
- 5회 실패 시 "계정이 일시적으로 잠겼습니다. 15분 후 다시 시도해주세요" 메시지
- 잠금 중 로그인 시도 시 "계정이 잠겨있습니다. X분 X초 후 다시 시도해주세요" 메시지

#### 엣지케이스
- 잘못된 이메일/비밀번호: 보안상 동일한 에러 메시지 ("이메일 또는 비밀번호가 올바르지 않습니다")
- 계정 잠금 (5회 실패): 15분 대기 안내, 타이머 표시
- 잠금 해제 시간 경과 후: 자동으로 login_attempts 0 리셋 및 잠금 해제
- 이메일 미인증 계정: Phase 0에서는 무시, Phase 1에서 인증 유도
- 네트워크 오류: "네트워크 연결을 확인해주세요" 안내
- 자동 로그인 미체크 시: 앱 종료 후 재실행 시 로그인 화면 표시
...
#### 입력
- 로그인 화면에서 "비밀번호 찾기" 링크 클릭
- 등록된 이메일 주소 입력

#### 처리
1. "비밀번호 찾기" 클릭 시 비밀번호 재설정 화면으로 이동
2. 이메일 입력 필드 제공
3. 이메일 형식 검증
4. "재설정 링크 보내기" 버튼 클릭:
   - 이메일 존재 여부 확인
...
5. 사용자가 이메일에서 링크 클릭:
   ...
   - 유효한 경우 새 비밀번호 입력 화면 표시
   - 무효한 경우 에러 화면 표시
6. 새 비밀번호 입력 화면:
   - 새 비밀번호 입력 필드 (동일한 검증 규칙)
   - 새 비밀번호 확인 입력 필드
...
7. "비밀번호 재설정" 버튼 클릭:
   ...
   - 토큰 사용 처리 (used: true)
   - 로그인 시도 횟수 0 리셋
   - 계정 잠금 해제 (locked_until: NULL)
8. 재설정 완료 후 로그인 화면으로 이동

#### 출력
- 이메일 입력 후 "재설정 링크가 발송되었습니다. 이메일을 확인해주세요" 메시지
- 토큰 무효 시 "재설정 링크가 만료되었거나 유효하지 않습니다" 에러 메시지
- 새 비밀번호 설정 성공 시 "비밀번호가 성공적으로 변경되었습니다" 메시지
- 로그인 화면으로 전환

#### 엣지케이스
- 존재하지 않는 이메일: 보안상 동일한 성공 메시지 표시 (이메일 존재 여부 노출 방지)
- 토큰 만료 (24시간 경과): "재설정 링크가 만료되었습니다. 다시 요청해주세요" 안내
- 토큰 이미 사용됨: "이미 사용된 재설정 링크입니다" 안내
- 새 비밀번호가 기존과 동일: 허용 (변경 이력 추적 없음)
- 비밀번호 재설정 중 앱 종료: 토큰 유효하므로 재진입 가능
- 여러 번 재설정 요청: 가장 최근 토큰만 유효 (기존 토큰 만료 처리)
