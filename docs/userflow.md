...
6. 전화 걸기 버튼 클릭 시
   - 디바이스 전화 앱 실행
   - 저장된 연락처로 전화 연결
7. 응급실 찾기 버튼 클릭 시 (선택적 기능)
   - 지도 앱 연동하여 주변 응급실 검색
8. 체크 기록 저장 (날짜시간, 선택 증상, 조치 여부)
9. 해당 없는 경우 화면 나가기 허용
...
### 엣지케이스
- 연락처 미등록: 설정으로 이동하여 등록 유도
- 여러 증상 동시 선택: 모두 기록하고 가장 심각한 증상 우선 표시
- 전화 앱 실행 실패: 연락처 복사 옵션 제공
- 지도 앱 미설치: 브라우저로 검색 또는 119 안내
- 증상 체크 후 조치하지 않고 나가기: 저장된 기록 유지
...
9. 퀵 액션 버튼 클릭 시 해당 기록 화면으로 이동
10. 리포트/뱃지 터치 시 상세 화면으로 이동

### 처리 (P1 - 향상 기능)
5. 데이터 기반 인사이트 자동 생성 **(P1)**
...
### 엣지케이스
- 전화번호 형식 오류: 에러 메시지 및 올바른 형식 안내
- 여러 연락처 등록 요청: 우선순위 1개만 저장 (MVP)
- 연락처 삭제: 삭제 확인 후 처리
- 저장 중 앱 종료: 변경사항 폐기
