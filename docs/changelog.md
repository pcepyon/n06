...
- [refactor] AI 메시지 프롬프트 개선 및 모델 변경
  - **모델 변경**: google/gemma-3n → openai/gpt-4o-mini
  - **프롬프트 구조**: combined → system/user 분리
  - **개선 사항**:
    - 대화 연속성 강조: 이전 메시지와 자연스럽게 연결
    - 데이터 직접 언급 금지: 숫자, 퍼센트 대신 자연스러운 표현
    - 금지 목록 강화: 이모지, 느낌표, 형식적 인사, 질문형 마무리
    - 역할 재정의: "동반자" → "이미 여러 번 대화한 친구"
