...
void main() {
  late ValidateWeightCreateUseCase useCase;

  setUp(() {
    useCase = ValidateWeightCreateUseCase();
  });

  group('ValidateWeightCreateUseCase', () {
    ...
    test('should be consistent with ValidateWeightEditUseCase', () {
      // Arrange & Act & Assert
      // 생성과 수정의 검증 로직이 동일해야 함
      final testWeights = [19.9, 20.0, 70.5, 300.0, 300.1];

      for (final weight in testWeights) {
        final result = useCase.execute(weight);
        
        if (weight < 20.0 || weight > 300.0) {
          expect(result.isFailure, true, reason: 'Weight $weight should fail');
        } else {
          expect(result.isSuccess, true, reason: 'Weight $weight should succeed');
        }
      }
    });
  });
